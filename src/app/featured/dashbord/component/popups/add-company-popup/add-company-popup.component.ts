...
export class AddCompanyPopupComponent implements OnInit {
  ...
  ngOnInit() {
    this.addUserForm = this.formBuilder.group({
      ...
      email: [(this.fields?.Email || ''), [Validators.email]],
      website: [(this.fields?.Website || '')],
      currency: [(this.fields?.CurrencyId || ''), Validators.required],
      retainedaccount: [(this.fields?.RetainedChainId || '')],
      accuretainedaccount: [(this.fields?.AccuRetainedAccountId || '')],
      erroraccount: [(this.fields?.SysErrorAccount || ''),],
      errortolerance: [(this.fields?.SysErrorTolerance || ''), Validators.required],
      manual: [(this.fields?.ManualPath || '')],
      comm: [(this.fields?.CommPath || '')],
